...
//!
//! **The SPI porting template.** `apps/imu` shows the three-layer stack for
//! I²C; this is its SPI counterpart — Layer 1 (`esp32-spi`) under Layer 2
//! (`spi-bus`), driven through the transfer-list [`Bus`] API. There is no
//! Layer-3 device here: the point is the bus, so it talks to itself.
//!
//! ```text
...
//! every byte clocked out arrives back on MISO — the same trick `apps/spidma`
//! and the on-target self-tests use. Order matters: MOSI first, then MISO, so
//! the second route wins the pad's input. SCK and a placeholder MISO come from
//! the board's spare loopback pads; `init` wants three distinct pins before
//! MISO is folded onto the scratch pad.
//!
//! # Porting to a real device
//!
//! Drop the fold, route MOSI/MISO/SCK to the device's pins, add a Layer-3
//! driver, and hand it a `BusHandle::new(&spi_bus)` exactly as `apps/imu` hands
//! one to its IMU driver. The Layer-1/Layer-2 bring-up below is unchanged.
