...
750 ms. It has no USB stack and does not use the ESP8285.
...
If picotool cannot be built on the host, add `-DPICO_NO_PICOTOOL=1`. That still
produces the inspectable ELF, BIN, HEX, map, and disassembly, but deliberately
does not produce a UF2.
...
## Hardware acceptance (requires explicit approval)
...
4. Confirm the GP13 LED pattern over repeated RUN resets.
