...
Invoke-Case 'test-pass-bootsel' @{ bootselSnapshots=@(@(), @(@{ instance_id='USB\VID_2E8A&PID_0003\PASS'; serial='PASS' })) } @('-Action','await-test-bootsel','-TimeoutSeconds','1','-BootselSerial','PASS') 0 '"state":"tests-passed"'
Invoke-Case 'test-timeout' @{ bootselSnapshots=@(@()) } @('-Action','await-test-bootsel','-TimeoutSeconds','1') 1 'did not enter RP2040 BOOTSEL'
Invoke-Case 'stale-bootsel' @{ bootselSnapshots=@(@(@{ instance_id='USB\VID_2E8A&PID_0003\STALE' })) } @('-Action','await-test-bootsel','-TimeoutSeconds','1') 1 'already present'
Invoke-Case 'wrong-bootsel-identity' @{ bootselSnapshots=@(@(), @(@{ instance_id='USB\VID_2E8A&PID_0003\OTHER'; serial='OTHER' })) } @('-Action','await-test-bootsel','-TimeoutSeconds','1','-BootselSerial','PASS') 1 'did not enter RP2040 BOOTSEL'
Invoke-Case 'wrong-bootsel-location' @{ bootselSnapshots=@(@(), @(@{ instance_id='USB\VID_2E8A&PID_0003\PASS'; serial='PASS'; location_path='USBROOT#USB(9)' })) } @('-Action','await-test-bootsel','-TimeoutSeconds','1','-BootselLocationPath','USBROOT#USB(2)') 1 'did not enter RP2040 BOOTSEL'
Invoke-Case 'ambiguous-bootsel' @{ bootselSnapshots=@(@(), @(@{ instance_id='USB\VID_2E8A&PID_0003\ONE'; serial='ONE' },@{ instance_id='USB\VID_2E8A&PID_0003\TWO'; serial='TWO' })) } @('-Action','await-test-bootsel','-TimeoutSeconds','1') 1 'multiple BOOTSEL devices matched'
