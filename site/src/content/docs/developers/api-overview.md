...
[**flintos API docs**](/api/api/). It is built
...
Everything an application calls lives in the [`api`](/api/api/) crate.

| Module | What it gives you |
|---|---|
| [`api::prelude`](/api/api/prelude/) | `use api::prelude::*;` — one glob for the logging macros, the `Task` builder, `sleep_ms`/`exit`/`wait_until`, `Error`/`Result`, the sync cells and the bus/pin/stream surface |
| [`api::task`](/api/api/task/) | The `Task` builder (`Task::new(name, entry).priority(..).stack(..).on_core(..).spawn()`), plus `spawn`/`spawn_on` free fns, `sleep_ms`, `yield_now`, `exit`, `wait_until` |
| [`api::queue`](/api/api/queue/) | Typed, bounded queues; `send`/`recv` with timeout, `send_isr` from an interrupt |
| [`api::mutex`](/api/api/mutex/) | `Mutex` with priority inheritance; `lock` returns a guard |
| [`api::sync`](/api/api/sync/) | `Once` and `CsCell` shared-static cells (also re-exported at the crate root) |
| [`api::timer`](/api/api/timer/) | `now_ms` and the monotonic tick |
| [`api::time`](/api/api/time/) | `now_us` — microsecond monotonic clock, for timing shorter than a tick |
| [`api::interrupt`](/api/api/interrupt/) | `connect(source, handler)` — route a peripheral IRQ to a top-half |
| [`api::dma`](/api/api/dma/) | `alloc`/`begin`/`await_transfer` for drivers that move blocks |
...
traits are the contract; the [rustdoc](/api/hal/bus/)
carries the exact signatures.

| You are writing | Implement | Reference |
|---|---|---|
| A sensor / display / LED (Layer 3) | uses a `BusHandle`, or a `lib/` device trait like `LedStrip` | [`hal::bus::BusHandle`](/api/hal/bus/#bushandle), [`led_strip`](/api/led-strip/) |
